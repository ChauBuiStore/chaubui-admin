...
import {
  ...
  CreateColorRequest,
  UpdateColorRequest,
} from "@/modules/colors/types/color.type";
...
export class ColorService {
  ...
  static async createColor(data: CreateColorRequest): Promise<Color> {
    const response = await httpClient.post<Color>(
      ENDPOINTS.COLOR.CREATE,
      data
    );
    return response.data;
  }
  ...
  static async deleteColor(id: string): Promise<void> {
    await httpClient.delete<void>(
      ENDPOINTS.COLOR.DELETE.replace(":id", id)
    );
  }

  static async bulkDeleteColors(
    ids: string[]
  ): Promise<{ success: boolean; data: unknown }> {
    const response = await httpClient.post<{ success: boolean; data: unknown }>(
      ENDPOINTS.COLOR.BULK_DELETE,
      { ids }
    );
    return response.data;
  }
