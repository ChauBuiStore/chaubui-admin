  ...
  return (
    <div className="text-right">
      <div className="flex items-center justify-end gap-2">
        <span
          className={`font-medium ${totalStock > 0 ? "text-primary" : "text-destructive"}`}
        >
          {totalStock}
        </span>
        <Popover>
                ...
                {product.variants && product.variants.length > 0 ? (
                  (() => {
                    ...
                    const groupedByColor = product.variants.reduce((acc, variant) => {
                      ...
                      return acc;
                    }, {} as Record<string, { color: Color | null, variants: ProductVariant[], totalStock: number }>);
                          ...
                          <div className="flex items-center gap-2">
                            {colorGroup.color ? (
                              <>
                                ...
                                />
                                <span className={`font-medium text-sm ${colorGroup.totalStock > 0 ? 'text-foreground' : 'text-destructive'}`}>
                                  {colorGroup.color.name}
                                </span>
                              </>
                            ) : (
                              <span className="font-medium text-sm text-muted-foreground">No Color</span>
                            )}
                          </div>
                          <div className="text-sm">
                            <span className={`font-medium ${colorGroup.totalStock > 0 ? 'text-primary' : 'text-destructive'}`}>
                        ...
                        <div className="divide-y">
                          {colorGroup.variants.map((variant, variantIndex) => (
                            <div key={variantIndex} className={`px-3 py-2 flex items-center justify-between text-sm ${variant.stock === 0 ? 'bg-destructive/5 opacity-75' : ''}`}>
                              <div className="flex items-center gap-2">
                                {variant.size?.name && (
                                  <span className={`px-2 py-0.5 text-xs rounded border ${variant.stock > 0
                                ...
                                {(() => {
                                  ...
                                  const discount = variant.discountPercent || (original > sale ? Math.round(((original - sale) / original) * 100) : 0);
                              ...
                              <div className="flex flex-col items-end gap-0.5 text-right">
                                <div className={`text-sm font-medium ${variant.originalPrice && variant.originalPrice !== variant.salePrice ? 'text-destructive' : ''
                                  } ${variant.stock === 0 ? 'text-muted-foreground' : ''}`}>
                                  {formatPrice(variant.salePrice)}
                                </div>
                                {variant.originalPrice && variant.originalPrice !== variant.salePrice && (
                                  <div className="text-xs text-muted-foreground line-through">
                                    {formatPrice(variant.originalPrice)}
                                  </div>
                                )}
                                <div className={`text-xs px-2 py-0.5 rounded ${variant.stock > 0 ? 'bg-primary/10 text-primary' : 'bg-destructive/10 text-destructive'
                                  }`}>
                                  {variant.stock} pcs
                                </div>
                              </div>
